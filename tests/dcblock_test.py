...
from single_endo import hh as hh_single
from couple_endo import hh as hh_couple
...
cali_sm = {'beta': 0.9833, 'vphi': 0.7625, 'chi': 0.5585, 'fU': 0.2499, 'fN': 0.1148, 's': 0.0203,
           'mean_z': 1.0, 'rho_z': 0.98, 'sd_z': 0.943*0.82, 'illiq': 0.25, 'pop': 0.29,
           'fU_eps': 10.69, 'fN_eps': 5.57, 's_eps': -11.17}

cali_sw = {'beta': 0.9830, 'vphi': 0.9940, 'chi': 0.4958, 'fU': 0.2207, 'fN': 0.1098, 's': 0.0132,
           'mean_z': 0.8, 'rho_z': 0.98, 'sd_z': 0.86*0.82, 'illiq': 0.15, 'pop': 0.29,
           'fU_eps': 8.72, 'fN_eps': 3.55, 's_eps': -6.55}

cali_mc = {'beta': 0.9882, 'illiq': 0.6, 'pop': 0.42, 'rho': 0.042,
...
to_map_single = ['beta', 'vphi', 'chi', 'fU', 'fN', 's', 'mean_z', 'rho_z', 'sd_z', 'transfer',
                 'fU_eps', 'fN_eps', 's_eps', *hh_single.outputs]
hh_sm = hh_single.remap({k: k + '_sm' for k in to_map_single}).rename('SingleMen')
hh_sw = hh_single.remap({k: k + '_sw' for k in to_map_single}).rename('SingleWomen')
...
to_map_couple = ['beta', 'transfer', 'rho',
                 ...
                 *hh_couple.outputs]
hh_mc = hh_couple.remap({k: k + '_mc' for k in to_map_couple}).rename('Couples')
