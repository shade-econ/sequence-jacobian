...
from sequence_jacobian import determinacy, get_H_U
...
def test_hank_determinacy(one_asset_hank_model):
    ...
    # Stable Case
    A = get_H_U(blocks, unknowns, targets, T, ss, asymptotic=True, save=True)
    wn = determinacy.winding_criterion(A)
    assert wn == 0

    # Unstable Case
    ss_unstable = {**ss, "phi": 0.75}
    A_unstable = get_H_U(blocks, unknowns, targets, T, ss_unstable, asymptotic=True, use_saved=True)
...
def test_two_asset_determinacy(two_asset_hank_model):
    ...
    A = get_H_U(blocks, unknowns, targets, T, ss, asymptotic=True)
    wn = determinacy.winding_criterion(A)
    assert wn == 0